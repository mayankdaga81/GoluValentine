...
button.addEventListener("click", () => {
  if (button.textContent === "Click Me! ❤") {
    ...
    fetch('send_mail.php')
      .then(response => {
        if (response.ok) {
          button.textContent = "I Love you, Golu Baby! ❤";
        } else {
...
function drawText() {
    ...
    // glow effect
    context.shadowColor = "rgb(244, 194, 194, 1)";
    context.shadowBlur = 8;
    context.shadowOffsetX = 0;
    context.shadowOffsetY = 0;

    if(frameNumber < 250){
        context.fillStyle = `rgb(244, 194, 194, ${opacity})`;
    ...
    if(frameNumber > 2500 && frameNumber < 99999){
        ...
        if (window.innerWidth < 600) {
            drawTextWithLineBreaks(["I love you so much Golu, more than", "all the time and space in the universe can contain"], canvas.width / 2, canvas.height / 2, fontSize, lineHeight);
        } else {
            context.fillText("I love you so much Golu, more than all the time and space in the universe can contain", canvas.width/2, canvas.height/2);
        }
    ...
    if(frameNumber >= 2750 && frameNumber < 99999){
        context.fillStyle = `rgb(244, 194, 194, ${secondOpacity})`;
    ...
    if(frameNumber >= 3000 && frameNumber < 99999){
        context.fillStyle = `rgb(244, 194, 194, ${thirdOpacity})`;
...
(function() {
    ...
    function showPlayButton() {
        if (document.getElementById('bg-play-button')) return;
        var btn = document.createElement('button');
        btn.id = 'bg-play-button';
    ...
    // Try on load, and again on first user interaction
